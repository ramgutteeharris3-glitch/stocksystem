    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 no-print">
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Net Sales (Incl. VAT)</p>
          <p className="text-3xl font-black text-indigo-600 tracking-tighter">MUR {totals.grossTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Tax Revenue (VAT 15%)</p>
          <p className="text-3xl font-black text-slate-800 tracking-tighter">MUR {totals.totalVat.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Taxable Base (Excl. VAT)</p>
          <p className="text-3xl font-black text-slate-800 tracking-tighter">MUR {totals.totalExcl.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-indigo-600 p-8 rounded-[2.5rem] shadow-xl text-white">
          <p className="text-[10px] font-black text-indigo-200 uppercase tracking-widest mb-2">Daily Volume</p>
      ...
      <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            <h2 className="text-2xl font-black text-slate-800 tracking-tight uppercase">Daily Sales Ledger</h2>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">
              Terminal: <span className="text-indigo-600">{currentShop}</span> • Official Record
            </p>
          </div>
          ...
          <div className="flex items-center gap-4 no-print">
             ...
             <button 
               ...
               className="px-6 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all flex items-center gap-2 shadow-lg"
             >
        ...
        <div className="mt-10 overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 text-slate-500 text-[9px] uppercase font-black tracking-widest border-y border-slate-100">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredLines.length > 0 ? filteredLines.map((item, idx) => (
                <tr key={`${item.receiptNumber}-${idx}`} className="hover:bg-slate-50 transition-colors">
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                      <span className="font-black text-indigo-600 text-xs">#{item.receiptNumber}</span>
                      <span className="text-[9px] font-bold text-slate-400">INV: {item.invoiceNumber}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="max-w-[200px]">
                      <p className="font-bold text-slate-800 text-xs uppercase truncate">{item.description}</p>
                      <p className="font-mono text-[9px] text-slate-400 uppercase">{item.productCode}</p>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <p className="text-[10px] font-black text-slate-600 uppercase tracking-tighter">{item.salesperson}</p>
                  </td>
                  <td className="px-8 py-6 text-right font-black text-slate-700">{item.quantity}</td>
                  <td className="px-8 py-6 text-right font-bold text-slate-500 text-xs">
                  ...
                  <td className="px-8 py-6 text-right font-black text-indigo-600 text-xs">
                    {item.lineTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </td>
                  <td className="px-8 py-6">
                    <span className="text-[8px] font-black uppercase text-indigo-500 bg-indigo-50 px-2 py-1 rounded-md border border-indigo-100">
                      {item.paymentMethod}
                    </span>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={8} className="px-8 py-32 text-center">
                    <div className="flex flex-col items-center justify-center opacity-30 grayscale">
                        <i className="fa-solid fa-receipt text-6xl mb-4 text-slate-300"></i>
                        <p className="font-black uppercase tracking-[0.4em] text-slate-400">Zero Ledger Entries</p>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 px-10">
          ...
          <div className="flex items-center gap-4 no-print">
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase">Synchronized with local storage</p>
          </div>
      </div>
    </div>
  );
